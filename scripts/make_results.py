...
def stage_rts1(gs, cs, du, gpucal):
    """S.1 скан по размерности, S.2 сертификаты, S.3 свод ветки RTS."""
    th = gs.get("thresholds", {})
    L = ["## RTS stage 1 — финальный скан по размерности (закрывающая стадия ветки)", "",
         "### Оценщик и его калибровка", "",
         "Шаг по состоянию решается **ADMM на GPU** (`scripts/rts_gpu.py`): проекция на конус PSD — `eigh` на "
         "видеокарте, проекция на аффинное множество (ω₁⊗ω₂ + Δ, Δ ∈ Anti⊗Anti, нулевые маргиналы Δ) — "
         "тензорные операции там же. Обе проекции точные и в замкнутом виде; это возможно потому, что образы "
         "сопряжённых маргинальных отображений ортогональны внутри Anti⊗Anti (антисимметричные матрицы "
         "бесследовы). Шаги по операциям остались на CPU в cvxpy. Причина перехода: при (6,6,6,6) один w-шаг "
         "в SCS занимал 349 с при том, что само `eigh` там 0.09 с — время уходило на канонизацию cvxpy и "
         "тысячи итераций SCS.", "",
         "**Важно для честности числа:** недосходимость ADMM не может завысить результат. Итоговое 𝒯 считается "
         "заново на строго допустимой точке после очистки, поэтому плохая сходимость только занижает найденное "
         "значение.", "",
         "| проверка оценщика | результат |", "|---|---|"]
    for k, v in gpucal.items():
        L.append(f"| {k} | {v} |")
    L += ["", "### S.1 Скан по размерности", "",
          "Отчётное число — очищенное 𝒯 (пофакторный шум: ISO и ОН точные, положительность по Холецкому).", "",
          "| d | PSD-блок N | лучшее 𝒯 очищ. | лучшее сырое | лучший старт | холодных | успешных | упавших | λmin | ISO | ОН | шум q | время |",
          "|---|---|---|---|---|---|---|---|---|---|---|---|---|"]
    for d in (2, 4, 6, 8):
        r = gs.get(f"d{d}")
        if not r:
        ...
        L.append(f"| {d} | {(d * d) ** 2} | **{r['T_clean_best']:.6f}** | {r['T_raw_best']:.6f} | {r['best_kind']} | "
                 ...
                 f"{c.get('q_noise', float('nan')):.1e} | {r['wall_seconds'] / 60:.0f} мин |")
    c8 = (gs.get("d8") or {}).get("ceiling")
    if c8:
        L += ["", "**Потолок по ресурсам при (8,8,8,8) — числами, без экстраполяции.** Строка d = 8 в таблице "
              "**не является результатом скана**: холодных стартов там нет вовсе, значение получено единственным "
              "тёплым стартом с недосошедшимся ADMM.", "", "| характеристика | значение |", "|---|---|"]
        L += [f"| {k} | {v} |" for k, v in c8.items()]
    L += ["", "| порог | значение |", "|---|---|",
          f"| потолок прогонов малой размерности 4+2√2 | {th.get('4+2sqrt2', 0):.6f} |",
          f"| вещественная граница Renou (RTW21, стр. 376) | {th.get('real_bound_RTW21')} |",
          f"| комплексное значение 6√2 | {th.get('6sqrt2', 0):.6f} |", ""]
    # независимая сверка CPU/SCS
    cp4 = cs.get("d4_partial_cpu_scs")
    if cp4:
        L += ["**Независимая сверка другим решателем.** Тот же see-saw через SCS на CPU (`rts_scan.py`): при "
              f"(2,2,2,2) — {cs['d2']['T_clean_best']:.6f} при {cs['d2']['n_success']} успешных из "
              f"{cs['d2']['n_attempts']} попыток (GPU-путь: {gs['d2']['T_clean_best']:.6f}, "
              f"{gs['d2']['n_success']} из {gs['d2']['n_success'] + gs['d2']['n_failed']}); при (4,4,4,4) — "
              f"{cp4['T_clean_best']:.6f} за {cp4['n_success']} стартов ({cp4['seconds_total'] / 3600:.1f} ч), "
              "прогон прерван по требованию освободить ресурсы. Значения совпадают при d = 2 и расходятся при "
              "d = 4: SCS-путь **ни разу** не нашёл точку уровня GPU-пути. Это свойство оценщика, а не класса "
              "процессов, и оно записано как ограничение.", ""]
    # S.2
    L += ["### S.2 Верхние оценки", "",
          "**Верхней оценки при свободных операциях получить не удалось.** Задача по операциям невыпуклая "
          "(𝒯 билинеен по состоянию и операциям), редукция по симметрии сценария не даёт выпуклой релаксации: "
          "функционал 𝒯 не инвариантен относительно обмена сторон (у Алисы 3 исхода, у Чарли 6), а локальные "
          "переименования уже учтены свободой операций. Поэтому вместо неё сделано более слабое, но **строгое** "
          "утверждение: при **найденных** операциях максимум по всем вещественным ISO-состояниям (ОН при этом "
          "даже не требуется — множество шире) ограничен сертификатом.", "",
          "| d | 𝒯 очищ. в точке | сертификат (ADMM, GPU) | зазор | λmin сертификата | сверка с cvxpy/SCS |",
          "|---|---|---|---|---|---|"]
    for d in (2, 4, 6, 8):
        ...
        altv = (f"{alt['certified_upper_bound']:.6f} (расх. "
                f"{abs(alt['certified_upper_bound'] - r['certified_upper_bound']):.1e})") if "certified_upper_bound" in alt \
            else "не собирается по памяти"
        L.append(f"| {d} | {r['T_at_point']:.6f} | **{r['certified_upper_bound']:.6f}** | {r['gap']:.1e} | "
                 f"{r['min_eig_certificate']:.1e} | {altv} |")
    L += ["",
          "Допустимость двойственной точки в каждой строке проверена разложением Холецкого после сдвига, "
          "поэтому это именно сертификат, а не значение солвера.", "",
          "**Что это даёт.** При d ≤ 6 зазор между найденным значением и сертификатом мал ("
          + ", ".join(f"d = {d}: {(du['gpu'][f'd{d}'])['gap']:.1e}" for d in (2, 4, 6) if f"d{d}" in du.get("gpu", {}))
          + "): see-saw выжимает состояние практически полностью при найденных операциях, и весь оставшийся "
          "зазор до 7.6605 приходится на **операции**, а не на поиск состояния. При d = 8 зазор на два порядка "
          "больше ("
          + f"{(du['gpu'].get('d8') or {}).get('gap', float('nan')):.1e}"
          + ") — это прямое следствие недосходимости ADMM в рамках бюджета, а не свойство задачи.", ""]
    cl = ((du.get("cvxpy") or {}).get("d4") or {}).get("CLARABEL", {})
    if "failed" in cl:
        L += ["**Внутренне-точный решатель.** Сверка Clarabel с SCS сделана при d = 2 (совпадение до 3e−9). "
              f"При d = 4 Clarabel неприменим: {cl['failed']}", ""]
    # интерпретация, зафиксированная в предрегистрации до прогонов
    v = {d: (gs.get(f"d{d}") or {}).get("T_clean_best") for d in (2, 4, 6, 8)}
    L += ["### Интерпретация (критерии зафиксированы в PREREGISTRATION_RTS1.md до прогонов)", "",
          f"Ход значений: d = 2 → {v[2]:.6f}, d = 4 → {v[4]:.6f}, d = 6 → {v[6]:.6f}"
          + (f", d = 8 → {v[8]:.6f} (не представительно, см. потолок)" if v.get(8) else "") + ".", "",
          "- **Монотонного роста к 6√2 = 8.485281 нет.** Значение колеблется в коридоре "
          f"[{min(x for k, x in v.items() if x and k < 8):.3f}, {max(x for k, x in v.items() if x and k < 8):.3f}] "
          "и не приближается ни к 7.6605, ни к 8.485281. Вывод «HW переносится» **не подтверждён**.",
          f"- **Строгий критерий плато не выполнен:** предрегистрация требовала совпадения соседних размерностей "
          f"в пределах 1e−3 (и 0.05 для пары d = 4, 6), а фактически |{v[4]:.6f} − {v[6]:.6f}| = "
          f"{abs(v[4] - v[6]):.3f}. Поэтому результат подаётся слабее, чем планировалось: не «плато», а "
          "**отсутствие роста** — три размерности подряд держатся около 6.83–6.93, тогда как переход от d = 2 "
          "к d = 4 дал всего +0.099, а от d = 4 к d = 6 — отрицательную разницу.",
          "- **Порог 7.6605 не превышен ни разу**, поэтому сценарий «это меняет вывод ветки» не наступил.", "",
          "Содержательно это численное свидетельство (не доказательство) в пользу того, что временная симметрия "
          "без селекции режет лазейку операциональной независимости: рост размерности фазового пространства, "
          "который в модели HW и даёт весь эффект, здесь ничего не прибавляет.", "",
          "### Прогнозы RTS stage 1", "", "| прогноз | уверенность | сбылся |", "|---|---|---|"]
    rows = [("(2,2,2,2): лучшее = 4+2√2", "0.9", abs(v[2] - (4 + 2 * 2 ** 0.5)) < 1e-3),
            ("(4,4,4,4): лучшее в [6.82, 6.95]", "0.7", 6.82 <= v[4] <= 6.95),
            ("(4,4,4,4): холодный не превзойдёт точку HW (6.869154) более чем на 0.05", "0.65",
             v[4] - 6.869154 <= 0.05),
            ("(6,6,6,6): прибавка к (4,4,4,4) меньше 0.1", "0.6", v[6] - v[4] < 0.1),
            ("плато: d = 4 и d = 6 в пределах 0.05 и ниже 7.6605", "0.55", abs(v[4] - v[6]) <= 0.05),
            ("ни при одной размерности 𝒯 не превысит 7.6605", "0.85",
             all(x < 7.6605 for x in v.values() if x)),
            ("ни при одной размерности 𝒯 не достигнет 6√2", "0.9",
             all(x < 6 * 2 ** 0.5 - 1e-6 for x in v.values() if x)),
            ("(8,8,8,8) не помещается в бюджет; потолок зафиксирован числами", "0.85", True),
            ("S.2: верхняя оценка при свободных операциях не будет получена", "0.8", True)]
    L += [f"| {a_} | {b_} | {ok(c_)} |" for a_, b_, c_ in rows]
    L += ["| S.2: редукция по симметрии уменьшит блок не более чем вчетверо | 0.6 | не проверялось: редукция не "
          "применялась, поскольку не даёт выпуклой релаксации |",
          f"| итог: ветка закрывается исходом «плато ниже 7.6605» | 0.55 | частично: значения ниже 7.6605, но "
          f"строгий критерий плато не выполнен (разброс {abs(v[4] - v[6]):.3f}) |", "",
          "**Прогноз архитектора** (при (8,8,8,8) значение заметно поднимется над 6.87, ~50%, с пометкой, что в "
          f"RTS 0 прогноз в ту же сторону не сбылся) — **НЕТ**: единственный уложившийся в бюджет старт дал "
          f"{v[8]:.6f}" + " при недосошедшемся ADMM; прогноз в ту же сторону не сбылся второй раз подряд.", ""]
    # S.3 — свод ветки
    L += ["### S.3 Свод ветки RTS", "",
          "**1. Что доказано (сертификаты, а не численные наблюдения).**",
          "- При операциях модели Hoffreumon–Woods максимум 𝒯 по **всем** вещественным ISO-процессам "
          "≤ 4.24264070 = 3√2 (двойственная точка, допустимость по Холецкому). Условия применимости: операции "
          "фиксированы (это операции HW); операциональная независимость **не требуется** — оценка верна на более "
          "широком множестве всех ISO-состояний. Комплексное значение той же конструкции — 6√2, то есть при "
          "переводе в ISO теряется ровно половина.",
          "- При найденных операциях скана (S.2) значение в точке и сертификат сходятся до "
          + ", ".join(f"{(du['gpu'][f'd{d}'])['gap']:.0e} (d = {d})" for d in (2, 4, 6) if f"d{d}" in du.get("gpu", {}))
          + ": состояние выжато, разрыв до порогов создают операции.", "",
          "**2. Механизм: почему конструкция HW не лежит в ISO.**",
          "6√2 набирается из четырёх ребитовых шаблонов — I и J_B1′J_B2′ дают по 2√2, J_A′J_C′ и "
          "J_A′J_B1′J_B2′J_C′ — по √2. Два из них ISO запрещает: **J_A′J_C′** (запрещён в TF — это корреляция "
          "выходов A и C, ресурс с будущей границы) и **J_B1′J_B2′** (запрещён в TB — корреляция входов Боба, "
          "ресурс с прошлой границы). Маргинальные куски этих шаблонов вклада в 𝒯 не несут: после их удаления "
          "𝒯 = 6√2 сохраняется, но λmin = −1/128, а восстановление положительности шумом съедает ровно "
          "прибавку (остаётся 2√2). В сетевом языке это фазовая опора, общая для сторон, не связанных общим "
          "источником; временная симметрия без селекции запрещает её с обеих сторон.", "",
          "**3. Что осталось открытым, в проверяемой постановке.**",
          "- Верхняя оценка sup 𝒯 по вещественным ISO-процессам с ОН при **свободных** операциях, любой "
          "размерности. Проверяемая формулировка: существует ли вещественный ISO-процесс с ОН и операции, "
          "дающие 𝒯 > 7.6605 (вещественная граница Renou)? Наш максимум — таблица выше.",
          "- Полное перечисление ISO₃ с составным входом Боба (мы работали внутри сетевой формы D12).",
          "- Поведение при d ≥ 10 и вопрос, выходит ли 𝒯 на плато или растёт логарифмически.", "",
          "**4. Отклонения и ограничения ветки.**",
          "- **Сужение до D12:** рассматривались процессы сетевой формы (ω на A⊗B1⊗B2⊗C, ISO ⇔ оба маргинала "
          "максимально смешаны), а не все ISO₃.",
          "- **Верхней оценки при свободных операциях нет** (S.2): задача по операциям невыпуклая, редукция по "
          "симметрии не даёт выпуклой релаксации — 𝒯 не инвариантен к обмену сторон (3 исхода у Алисы против 6 "
          "у Чарли).",
          "- **Оценщик даёт нижние оценки.** See-saw находит локальные максимумы; сверка двух независимых "
          "решателей (SCS на CPU и ADMM на GPU) при d = 4 показала, что SCS-путь систематически находит худшие "
          "точки. Поэтому числа таблицы — достигнутые значения, а не максимумы класса.",
          "- **Доля упавших стартов** на SCS-пути высока (при (2,2,2,2) — 22 из 42 попыток); на GPU-пути "
          "отказов нет. Упавший старт — отказ солвера на каком-либо шаге, он отбрасывается целиком.",
          "- **Clarabel неприменим** при N ≥ 256: плотный блок гессиана PSD-конуса svec(256)² требует "
          "многогигабайтного выделения одним куском, отказ происходит в аллокаторе Rust и обрывает процесс. "
          "MOSEK в окружении нет. Сверка внутренне-точного решателя с первопорядковым сделана при d = 2.",
          "- **Потолок по ресурсам зафиксирован числами** (см. таблицу скана и раздел про d = 8), "
          "экстраполяция вместо счёта не применялась.",
          "- **Точка при d = 8 не в репозитории:** файл `results/rts_gpu_8.npz` весит 129 МБ и внесён в "
          "`.gitignore`; воспроизводится скриптом из `scripts/run_all.sh`. Точки d = 2, 4, 6 сохранены.",
          "- **Сбой окружения в RTS 0:** плотный базис Δ (~7,5 ГБ) вместе с параллельным прогоном уронил систему "
          "пользователя. После этого базис разреженный, у каждого скрипта RLIMIT_AS, расчёты идут по одному; "
          "в GPU-пути базис не нужен вовсе — проекции безбазисные.", ""]
    return L
